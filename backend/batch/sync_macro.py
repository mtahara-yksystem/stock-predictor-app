...
from app.fetcher.yfinance_fetcher import MACRO_TICKERS, YFinanceFetcher
...
def run_macro_sync():
    ...
    latest_dates = [repo.get_latest_date(ticker) for ticker in MACRO_TICKERS.keys()]
    latest_dates = [d for d in latest_dates if d is not None]

    if not latest_dates:
        # 初回: 過去1825日分を全取得
        print("🆕 初回取得: 過去1825日分のマクロ指標を取得します...")
        df = fetcher.fetch_macro_indicators(days_back=1825)
    else:
        # 差分取得: 最も古い最新日の翌日から取得
